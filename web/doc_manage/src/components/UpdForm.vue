...
const props = defineProps({
  ...
  optionsSelect: { type: Object, required: true },
...
<template>
  <form method="POST" class="needs-validation" novalidate enctype="multipart/form-data" @submit.prevent>
    <div class="row g-3">
      ...
      <div class="col-sm-6">
        <select class="form-floating form-select" v-model="attrs.categories" @change="onChangeCategory($event)">
          <option disabled selected>Способ заключения</option>
          <option v-for="item in optionsSelect.categories" :key=item.category_id>{{
            item.category_name }}
          </option>
        </select>
      </div>
      <div class="col-sm-6">
        <select class="form-floating form-select" v-model="attrs.distributors" @change="onChangeDescription($event)">
          <option disabled selected>Поставщик</option>
          <option v-for="item in optionsSelect.distributors" :key=item.distributor_id :value="item.distributor_name">{{
